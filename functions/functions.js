...
export const preflopround = async (room, io) => {
  ...
  await updateGamebyTableId(room.tableId, room.gameType, "inGame");
  ...
  if (!room.finish && !room.gamestart) {
    if (room.runninground === 0 && !room.pause) {
      ...
      if (
        ...
        while (smallBlindDeducted < 1) {
          ...
          if (playerAvilable.length) {
            ...
            if (smallLoopTime < totalplayer) {
              ...
              io.in(room.tableId.toString()).emit("notification", {
        ...
        io.in(room.tableId.toString()).emit("preflopround", updatedRoom);
      } else {
        io.in(room.tableId.toString()).emit("onlyOnePlayingPlayer", {
          msg: "Game finished, Only one player left",
          roomdata: room,
        });
        if (room.gameType === "pokerTournament_Tables") {
          await finishedTableGame(room);
          io.in(room.tableId.toString()).emit("roomFinished", {
            msg: "Game finished",
            finish: room.finish,
            roomdata: room,
          });
      ...
      io.in(room.tableId.toString()).emit("tablestopped", {
...
export const showdown = async (roomid, io) => {
  ...
  upRoomData.showdown.forEach((player, i) => {
    let action, amt;
    if (player.playing) {
      if (winnerPlayers.find((ele) => ele.id === player.id)) {
        action = "game-win";
        if (updatedRoom.allinPlayers.find((allin) => allin.id === player.id)) {
          if (
            winnerPlayers.find((ele) => ele.id === player.id).winningAmount -
              updatedRoom.allinPlayers.find((allin) => allin.id === player.id)
                .amt <
            0
          ) {
            action = "game-lose";
          ...
          amt = winnerPlayers.find((ele) => ele.id === player.id).winningAmount;
        }
      } else {
        action = "game-lose";
        amt = player.prevPot;
      ...
      each(
        ...
        async function (el, next) {
          ...
          let hands = el.hands.filter(
            (hand) => hand.action !== "game-lose" && hand.action !== "game-win"
          );
          let buyinchips = 0;
          let stripeBuy = hands;
          ...
          if (haveleave.length) {
            ...
            let stats = await getUpdatedStats(uid);
            newHandPlayer.push({
              userid: uid,
              name: el.name,
  ...
  try {
    if (isValid) {
      ...
      if (roomData) {
        ...
        switch (roomData.runninground) {
          case 0:
            sitOut.push(roomData.players.find((el) => el.userid === userid));
          ...
          case 1:
            sitOut.push(roomData.preflopround.find((el) => el.id === userid));
            updatedData = await roomModel.findOneAndUpdate(
              {
                _id: roomid,
          ...
          case 2:
            sitOut.push(roomData.flopround.find((el) => el.id === userid));
            updatedData = await roomModel.findOneAndUpdate(
              {
                _id: roomid,
                "flopround.id": userid,
          ...
          case 3:
            sitOut.push(roomData.turnround.find((el) => el.id === userid));
            updatedData = await roomModel.findOneAndUpdate(
              {
                _id: roomid,
                "turnround.id": userid,
          ...
          case 4:
            sitOut.push(roomData.riverround.find((el) => el.id === userid));
            updatedData = await roomModel.findOneAndUpdate(
              {
                _id: roomid,
    ...
    if (isValid) {
      ...
      if (roomdata) {
        ...
        if (roomdata.hostId === userid) {
          let p = roomdata.players.filter((ele) => ele.userid !== userid)[0];
          if (p) {
            roomdata.players
      ...
      async function (player, next) {
        if (dPlayers.length) {
          ...
          if (havePosition.length) {
            ...
            // console.log("player sit done on position ==>", player.position);
    ...
    if (room) {
      ...
      if (prebet) {
        ...
        if (update) {
          ...
          io.in(tableId.toString() + "watchers").emit("newBetPlaced", update);
          io.in(tableId.toString()).emit("watcherbet", data);
        }
      } else {
        ...
        socket.emit("betCreated");
        io.in(tableId.toString() + "watchers").emit("newBetPlaced", newbet);
        io.in(tableId.toString()).emit("watcherbet", data);
      }
    } else {
...
export const handleWatcherWinner = async (room, io) => {
  try {
    ...
    let bets = await BetModal.findOne({ tableId: room.tableId });
    let roomWatcher = await roomModel.findOne({ tableId: room.tableId });
    ...
    if (bets) {
      ...
      io.in(room.tableId.toString() + "watchers").emit("watcherWinners", {
      ...
      await BetModal.deleteOne({ tableId: room.tableId });
      await roomModel.updateOne(
...
export const addBuyIn = async (
  ...
  try {
    ...
    if (room && room.watchers.find((ele) => ele.userid === userId)) {
      ...
      if (updateRoom) {
        io.in(tableId.toString()).emit("CoinsAdded", {
          userId,
...
export const doLeaveWatcher = async (data, io, socket) => {
  try {
    ...
    if (isCalled) {
      ...
      if (updatedRoom) {
        io.in(tableId.toString()).emit("updatePlayerList", updatedRoom);
        setTimeout(() => {
          socket.emit("reload");
        }, 30000);
      }
...
export const leaveApiCall = async (room, userId) => {
  try {
    ...
    if (
      !player.find((el) => (el.id ? el.id === userId : el.userid === userId)) &&
...
export const finishHandApiCall = async (room, userId) => {
  try {
    ...
    console.log("Res of finish hand api =>", res.data);
    if (res.data.error === "no error") {
      let newPlayers = [];
