...
const addUserInSocket = (io, socket, gameId, userId) => {
  ...
  io.room = [...new Set(lastSocketData.map((ele) => ele.room.toString()))].map(
    (el) => ({
      room: el,
      pretimer: false,
    }),
  ...
  if (!room.finish && !room.gamestart) {
    ...
    if (room.runninground === 0 && !room.pause) {
      ...
      if (playingPlayer.length > 1) {
        ...
        console.log("Line 715 only one playing player")
        ...
        if (room.gameType === 'pokerTournament_Tables') {
          console.log("Line 721 finished table game")
          await finishedTableGame(room)
          console.log("Line 721 room finished")
      ...
      console.log("Line 732 table stoped")
      io.in(room._id.toString()).emit('tablestopped', {
        msg: 'Game paused by host',
      })
    }
  } else {
    console.log('Line 738 table game finished-->')
    io.in(room._id.toString()).emit('tablestopped', {
...
export const prefloptimer = async (roomid, io) => {
  console.log("Line 746 preflop timer")
...
export const flopround = async (roomid, io) => {
  console.log("Line 979 flop round")
  const roomData = await roomModel.findOne({ _id: roomid })
...
export const showdown = async (roomid, io) => {
  ...
  setTimeout(async () => {
    ...
    if (roomUpdate.finish) {
      console.log('Line 2298 Game finished ')
      io.in(roomUpdate._id.toString()).emit('roomFinished', {
        msg: 'Game finished',
        finish: roomUpdate.finish,
        roomdata: roomUpdate,
      ...
      if (roomData) {
        ...
        switch (roomData.runninground) {
          ...
          case 1:
            ...
            console.log("<<-----first winnerBeforeshowdown----->>")
          ...
          case 2:
            ...
            if (playingPlayer.length === 1) {
              ...
              console.log("<<-----second winnerBeforeshowdown----->>")
              await winnerBeforeShowdown(
                roomid,
                playingPlayer[0].id,
                roomData.runninground,
          ...
          case 3:
            ...
            if (playingPlayer.length === 1) {
              ...
              console.log("<<-----third winnerBeforeshowdown----->>")
              await winnerBeforeShowdown(
                roomid,
                playingPlayer[0].id,
                roomData.runninground,
          ...
          case 4:
            ...
            if (playingPlayer.length === 1) {
              ...
              console.log("<<-----fourth winnerBeforeshowdown----->>")
              await winnerBeforeShowdown(
                roomid,
                playingPlayer[0].id,
                roomData.runninground,
    ...
    switch (roomData.runninground) {
      case 1:
        ...
        if (playingPlayer.length === 1) {
          ...
          console.log("<<-----fifth winnerBeforeshowdown----->>")
          await winnerBeforeShowdown(
            roomid,
            playingPlayer[0].id,
            roomData.runninground,
      ...
      case 2:
        ...
        if (playingPlayer.length === 1) {
          ...
          console.log("<<-----sixth winnerBeforeshowdown----->>")
          await winnerBeforeShowdown(
            roomid,
            playingPlayer[0].id,
            roomData.runninground,
      ...
      case 3:
        ...
        if (playingPlayer.length === 1) {
          ...
          console.log("<<-----seventh winnerBeforeshowdown----->>")
          await winnerBeforeShowdown(
            roomid,
            playingPlayer[0].id,
            roomData.runninground,
      ...
      case 4:
        ...
        if (playingPlayer.length === 1) {
          ...
          console.log("<<-----eighth winnerBeforeshowdown----->>")
          await winnerBeforeShowdown(
            roomid,
            playingPlayer[0].id,
            roomData.runninground,
...
export const socketDoAllin = async (dta, io, socket) => {
  ...
  try {
    if (isValid) {
      ...
      console.log({ data })
      if (data !== null) {
        console.log('=================== ALLIN 4547')
        await doAllin(roomid, playerid, io)
      } else {
        console.log('=================== ALLIN 4550')
        socket.emit('actionError', { code: 404, msg: 'Data not found' })
      }
    } else {
      console.log('=================== ALLIN 4554')
      socket.emit('actionError', { code: 400, msg: 'Bad request' })
    }
  } catch (e) {
    console.log('error : ', e)
...
const winnerBeforeShowdown = async (roomid, playerid, runninground, io) => {
  const roomData = await roomModel.findOne({ _id: roomid })
  console.log('ROOM DATA PLAYERS 4565')
  let winnerAmount = 0
  let showDownPlayers = []
  let playerData = null
  let totalPot = roomData.pot
  ...
  setTimeout(async () => {
    ...
    console.log("I am here--- for check re arrange table")
      ...
      async function (player, next) {
        if (dPlayers.length) {
          ...
          if (havePosition.length) {
            ...
            io.in(roomData._id.toString()).emit('roomData', rUpdatedData)
            console.log("<<<----Line number 5459 room change---->>>",rUpdatedData)
        ...
        if (room.table.public) {
          ...
          if (
            ...
            if (deduct) {
              ...
              io.in(room.roomid.toString()).emit('updatePlayerList', updateRoom)
              await removeInvToPlayers(room.roomid, user.userid, gameType)
            } else {
              io.in(room.roomid.toString()).emit('lowBalance', {
                userid: user.userid,
        ...
        if (room.players.length < 4) {
          ...
          if (joinPlayer.length) {
            ...
            if (deduct) {
              ...
              await removeInvToPlayers(data._id, userData.userid, data.gameType)
            } else {
              io.in(data._id.toString()).emit('lowBalance', {
                userid: userData.userid,
              })
...
export const updateSeenBy = async (data, socket, io) => {
  console.log('update chat is read ')
  try {
...
const pushPlayerInRoom = async (
  ...
  if (checkTournament?.rooms?.length && lastRoom?.players?.length < 4) {
