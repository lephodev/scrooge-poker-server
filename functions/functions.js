...
export const gameTurnTimer = async (roomid, io) => {
  try {
    ...
    console.log("game round", state, roomid);
        ...
        each(
          ...
          async function (err, transformedItems) {
            ...
            try {
              ...
              await setCachedGame(roomData);
              const updatedroom = await getCachedGame(roomData._id);
              console.log(
      ...
      if (!roomData.finish) {
        ...
        if (roomData.runninground === 0) {
          ...
          if (checkRoom && checkRoom.length > 2) {
            ...
            if (!roomData.finish) {
              await setCachedGame({ _id: roomData._id, finish: true });
              await roomModel.updateOne(
                { _id: roomData._id },
                { finish: true }
              );
            ...
            await setCachedGame({ _id: roomData._id, pause: true });
    ...
    if (isValid) {
      ...
      if (roomData) {
        ...
        if (
          ...
          await setCachedGame({ _id: roomData._id, runninground: 5 });
...
export const startPreflopRound = async (data, socket, io) => {
  try {
    ...
    if (room && !room.gamestart) {
      await setCachedGame({ _id: room._id, pause: false });
      await preflopround(room, io);
    }
  } catch (e) {
        ...
        el.hands.forEach((elem) => {
          ...
          if(room.gameMode !== "goldCoin"){
            if(user.dailySpinBonus >= betAmount){
              ...
              if(action === 'game-win'){
                const winAmtForMonthly = (mnthlyPercntage/100) * amount;
