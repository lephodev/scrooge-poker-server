  ...
  try {
    ...
    if (!room.finish) {
      ...
      if (room.runninground === 0) {
        console.log("inside running round player length", playingPlayer.length);
        if (playingPlayer.length > 1) {
          ...
          console.log("maxPosition ==>", totalplayer);
            ...
            try {
              ...
              if (
                ...
                room.tournament.eleminatedPlayers.find(
                  (el) => el.userid.toString() === x.userid.toString() || el.id ? el.id : el.userid === x.id ? x.id : x.userid)
                )
              ) {
                continue;
              }
...
export const doAllin = async (roomData, playerid, io) => {
  try {
    let { raiseAmount, raisePlayerPosition, allinPlayers, runninground } = roomData;
    if (roomData?.timerPlayer?.toString() === playerid?.toString()) {
  ...
  try {
    ...
    if (blankSpot >= totalPlayers) {
      ...
      if (room.showdown.length > 1) {
        ...
        await setCachedGame({...room, players: room.showdown, runninground:0, gamestart:false, communityCard: []});
...
const createTransactionFromUsersArray = async (
  ...
  try {
    ...
    for await (const user of users) {
      ...
      console.log("current users wallet ==>", crrUser.goldCoin);
...
export const leaveApiCall = async (room, userId, io) => {
  try {
    ...
    if (userId) {
      ...
      await setCachedGame({...rrr, players: room.players, watchers: room.watchers, handWinner: filterdHndWinnerData});
